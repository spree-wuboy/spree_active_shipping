  ...
  class UPS < Carrier
    ...
    def commit(action, request, test = false)
      headers = {
        ...
        "Authorization" => "Bearer #{get_ups_access_token(test)}"
      }
      url = "#{test ? TEST_URL : LIVE_URL}/#{RESOURCES[action]}"
      Rails.logger.info("UPS Request: \nurl: #{url}\nrequest: #{request}\nheaders: #{headers}")
    ...
    def find_time_in_transit(origin, destination, packages, options={})
      ...
      response = commit(:delivery_dates, save_request(rate_request), (options[:test] || false))
    ...
    def get_ups_access_token(test=false)
      require 'rest-client'
      require 'json'
      url = "#{test ? TEST_URL : LIVE_URL}/security/v1/oauth/token" # use https://onlinetools.ups.com/... for production
      ...
      puts "Failed to get UPS access token"
      puts e.response
      nil
    end
  end
